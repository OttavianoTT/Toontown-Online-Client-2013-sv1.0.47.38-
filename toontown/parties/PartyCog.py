...
class PartyCog(FSM):
    ...
    def showHitScore(self, number, scale = 1):
        ...
        self.partyCogHpTextSeq = Sequence(self.hpText.posInterval(0.25, Point3(self.root.getX(render), self.root.getY(render), self.root.getZ(render) + self.height + 1.0), blendType='easeOut'), Wait(0.25), self.hpText.colorInterval(0.1, Vec4(r, g, b, 0)), Func(self.hideHitScore))
        self.partyCogHpTextSeq.start()
    ...
    def hideHitScore(self):
        if self.partyCogHpTextSeq:
            self.partyCogHpTextSeq.finish()
            self.partyCogHpTextSeq = None
        if self.hpText:
