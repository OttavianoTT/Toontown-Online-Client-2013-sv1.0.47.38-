...
try:
    ...
    print('ToontownStart: Failed to start injector - wx module missing!')
else:
    try:
        import psutil
    except:
        print('ToontownStart: Failed to start injector - psutil module missing!')
    else:
        from toontown.toonbase.ToontownInjector import ToontownInjector
        print('ToontownStart: Starting injector...')
...
if autoRun and launcher.isDummy() and (not Thread.isTrueThreads() or __name__ == '__main__' or 'toontown.toonbase.ToontownStart'):
    try:
        base.run()
    except SystemExit:
        raise
