...
class DistributedClosetAI(DistributedFurnitureItemAI):
    ...
    def enterAvatar(self):
        ...
        if not owner:
            self.air.dbInterface.queryObject(self.air.dbId, self.ownerId, self.__handleOwnerQuery, self.air.dclassesByName['DistributedToonAI'])
            return
        ...
        taskMgr.doMethodLater(ClosetGlobals.TIMEOUT_TIME, self.__handleClosetTimeout, 'closet-timeout-%d' % avId, extraArgs=[
         avId])
    ...
    def removeItem(self, dnaString, itemType):
        ...
        if itemType == ClosetGlobals.SHIRT:
            ...
            if itemType == ClosetGlobals.SHORTS:
        ...
        if finished == 1:
            ...
            if finished == 2:
                ...
                if whichItems & ClosetGlobals.SHIRT:
                    if av.replaceItemInClothesTopsList(testDNA.topTex, testDNA.topTexColor, testDNA.sleeveTex, testDNA.sleeveTexColor, self.customerDNA.topTex, self.customerDNA.topTexColor, self.customerDNA.sleeveTex, self.customerDNA.sleeveTexColor):
                ...
                if whichItems & ClosetGlobals.SHORTS:
                    if av.replaceItemInClothesBottomsList(testDNA.botTex, testDNA.botTexColor, self.customerDNA.botTex, self.customerDNA.botTexColor):
