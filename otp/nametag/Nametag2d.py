...
class Nametag2d(Nametag, MarginPopup):
    ...
    def __init__(self):
        ...
        self.contents = self.CName | self.CSpeech | self.CExclaim
    ...
    def considerUpdateClickRegion(self):
        ...
        if self.isDisplayed():
            left, right, bottom, top = self.frame
            self.updateClickRegion(left*self.SCALE, right*self.SCALE,
                                   bottom*self.SCALE, top*self.SCALE)
        else:
    ...
    def getExclaimBalloon(self):
        return NametagGlobals.exclaimBalloon2d
