...
# --- ACCOUNT DATABASES ---
# These classes make up the available account databases for Toonsanity.
...
class AccountDB:
    ...
    def __init__(self, clientManager):
        ...
        FOLDER_PATH = 'astron/databases/astrondb'

        if not os.path.exists(FOLDER_PATH):
            os.makedirs(FOLDER_PATH)

        filename = simbase.config.GetString('account-bridge-filename', 'astron/databases/account-bridge.db')
...
class LoadAvatarFSM(AvatarOperationFSM):
    ...
    def enterSetAvatar(self):
        ...
        self.clientManager.air.toonsanityFriendsManager.comingOnline(self.avId, friendsList)
        ...
        # unexpectedly.
        if self.clientManager.air.toonsanityFriendsManager:
            friendsManagerDclass = self.clientManager.air.toonsanityFriendsManager.dclass
            cleanupDatagram = friendsManagerDclass.aiFormatUpdate('goingOffline',
                                                                  self.clientManager.air.toonsanityFriendsManager.doId,
                                                                  self.clientManager.air.toonsanityFriendsManager.doId,
                                                                  self.clientManager.air.ourChannel, [self.avId])
        else:
            friendsManagerDoId = OtpDoGlobals.OTP_DO_ID_TOONSANITY_FRIENDS_MANAGER
            friendsManagerDclass = self.clientManager.air.dclassesByName['ToonsanityFriendsManagerUD']
...
class UnloadAvatarFSM(OperationFSM):
    ...
    def enterUnloadAvatar(self):
        ...
        self.clientManager.air.toonsanityFriendsManager.goingOffline(self.avId)
